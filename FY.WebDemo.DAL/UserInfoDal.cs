...
namespace FY.WebDemo.DAL
{
    public class UserInfoDal
    {
        ...
        {
            string sql = "insert into UserInfo(UserName,Password,RegTime,Email) values(@UserName,@Password,@RegTime,@Email)";
            SqlParameter[] pars = { 
        ...
        public int EditUserInfo(UserInfo userInfo)
        {
            ...
            string sql = "update UserInfo set UserName=@UserName,Password=@Password,RegTime=@RegTime,Email=@Email where ID=@ID";
            SqlParameter[] pars = { 
                                new SqlParameter("@UserName",SqlDbType.NVarChar,32),
                                  new SqlParameter("@Password",SqlDbType.NVarChar,32),
                                         new SqlParameter("@RegTime",SqlDbType.DateTime),
        ...
        {
            string sql = "select * from (SELECT *,Row_Number() over(order by Id) as rownum FROM UserInfo)as t where t.rownum>@startRowIndex and t.rownum<=@startRowIndex+@maximumRows";
            SqlParameter[] pars = { 
                                  new SqlParameter("@startRowIndex",SqlDbType.Int),
                                  new SqlParameter("@maximumRows",SqlDbType.Int)
                                  };
            pars[0].Value = startRowIndex;
            pars[1].Value = maximumRows;
            DataTable da = SqlHelper.GetDataTable(sql, CommandType.Text, pars);
            List<UserInfo> list = null;
            if (da.Rows.Count > 0)
            {
